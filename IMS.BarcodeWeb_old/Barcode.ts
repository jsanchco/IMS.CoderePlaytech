...
//based on http://angularscript.com/angularjs-directive-for-code-128-barcode-generator/
            ...
            for (var y = 0, x; y < lines; y++) {
                ...
                for (x = 0; x < columns; x++) {
                    if (current == digit[y][x]) {
                        ...
                        content += (current == '0' ? bar0 : bar1).replace('%s', (len * mw).toString());
                        current = digit[y][x];
                        len = 1;
                    }
                }
    ...
    ////
    generate = function (datas, type, settings) {
        ...
        return this.barcode[fname](this.barcode.code128, settings, digit, hri, type);
    };
    ...
    public getBarCode(value: any): string {
        console.log("getBarCode")
        return this.generate(value, "code128", { barWidth: 2 }).outerHTML
    }
